...
var ZeekIntelDesc = `Zeek Intelligence data matches
Reference: https://docs.zeek.org/en/current/scripts/base/frameworks/intel/main.zeek.html#type-Intel::Info`
...
// ZeekIntelParser parses zeek ssh logs
type ZeekIntelParser struct{}

func (p *ZeekIntelParser) New() parsers.LogParser {
	return &ZeekIntelParser{}
}

// Parse returns the parsed events or nil if parsing failed
func (p *ZeekIntelParser) Parse(log string) []*parsers.PantherLog {
	ZeekIntel := &ZeekIntel{}

	err := jsoniter.UnmarshalFromString(log, ZeekIntel)
	if err != nil {
		zap.L().Debug("failed to parse log", zap.Error(err))
		return nil
	}

	ZeekIntel.updatePantherFields(p)

	if err := parsers.Validator.Struct(ZeekIntel); err != nil {
		zap.L().Debug("failed to validate log", zap.Error(err))
		return nil
	}

	return ZeekIntel.Logs()
}

// LogType returns the log type supported by this parser
func (p *ZeekIntelParser) LogType() string {
	return "Zeek.SSL"
}

func (event *ZeekIntel) updatePantherFields(p *ZeekIntelParser) {
